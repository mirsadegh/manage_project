...
class TeamViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # ✅ Allow 'join' action to access any team (needed for self-join)
        if self.action == 'join':
            return Team.objects.filter(allow_self_join=True) 
    ...
    def join(self, request, pk=None):
        ...
        for lead_membership in team.memberships.filter(role=TeamMembership.Role.LEAD, is_active=True):
            Notification.objects.create(
                recipient=lead_membership.user,
